...
def open_book(book_name=None):
    try:
        ...
        if os.path.exists(book_name):
            print('Данный файл уже существует, желаете его открыть?')
            if bin_answer():
...
def manage_book(book):
    while True:
        ...
        if book.have_changes:
            print('* - есть несохранённые изменения, хотите сохранить?')
            if bin_answer():
                book.save()
...
def edit_contact(book, delete=False):
    if len(book):
        print(f'Нужно выбрать контакт для {"удаления" if delete else "редактирования"}, хотите осуществить поиск?')
        if bin_answer():
